...
# OpenVINO 모델 로딩
core = ov.Core()
model = core.read_model("./model/model01.xml", weights="./model/model01.bin")
...
LABEL_NAMES = {0: "CRACK"}
COLORS = {0: (0, 255, 0)}  # CRACK 라벨에 대한 색상
...
def process_output(boxes_output, labels_output, frame_shape):
    ...
    for i in range(boxes_output.shape[1]):
        ...
        if conf > 0.4:
            ...
            x_scale = w / frame_shape[1]
            y_scale = h / frame_shape[0]
...
try:
    while True:
        ...
        # CRACK 검출 후 이미지 저장 (단 한번만 저장)
        if crack_detected and not image_saved:
