...
# 볼트 모델 로드
bolt_model = core.read_model("./model/Bolt/openvino.xml", weights="./model/Bolt/openvino.bin")
...
# 크랙 모델 로드 (새로운 모델 추가)
crack_model = core.read_model("./model/Crack/model.xml", weights="./model/Crack/model.bin")  # 경로 수정 필요
...
try:
    while True:
        ...
        bolt_boxes = process_output(bolt_output["boxes"], bolt_output["labels"], roi.shape, (W_bolt, H_bolt))
        ...
        crack_boxes = process_output(crack_output["boxes"], crack_output["labels"], roi.shape, (W_crack, H_crack))
        ...
        for box in crack_boxes:
            ...
            cv2.putText(frame, f"{LABEL_NAMES['crack'][label]} {conf:.2f}", 
                       (x1, y1-50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
