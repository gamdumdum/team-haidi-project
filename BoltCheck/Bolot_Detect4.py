...
bolt_compiled = core.compile_model(
    model=bolt_model, device_name="GPU.1"
)  # 외장 그래픽 카드
N_bolt, C_bolt, H_bolt, W_bolt = bolt_compiled.input(0).shape

# 크랙 모델 로드 [ 경로 확인 ]
crack_model = core.read_model(
    "./model/Crack/model_test.xml", weights="./model/Crack/model_test.bin"
)
...
crack_compiled = core.compile_model(
    model=crack_model, device_name="GPU.1"
)  # 외장 그래픽 카드
...
# 라벨 및 색상 설정
LABEL_NAMES = {"bolt": {1: "Bolt_OK", 0: "Bolt_NG"}, "crack": {0: "Crack"}}
...
try:
    ...
    while True:
        ...
        input_tensor1 = np.expand_dims(resized1.transpose(2, 0, 1), 0).astype(
            np.float32
        )
        ...
        input_tensor3 = np.expand_dims(resized3.transpose(2, 0, 1), 0).astype(
            np.float32
        )
        input_tensor4 = np.expand_dims(resized4.transpose(2, 0, 1), 0).astype(
            np.float32
        )
        ...
        if ret1:
            ...
            bolt_boxes1 = P_output.process_output(
                bolt_output1["boxes"],
                bolt_output1["labels"],
                roi1.shape,
                W_bolt,
            ...
            crack_boxes1 = P_output.process_output(
                crack_output1["boxes"],
                crack_output1["labels"],
                roi1.shape,
                W_crack,
                H_crack,
            )
        ...
        if ret2:
            ...
            bolt_boxes2 = P_output.process_output(
                bolt_output2["boxes"],
                bolt_output2["labels"],
                roi2.shape,
                W_bolt,
            ...
            crack_boxes2 = P_output.process_output(
                crack_output2["boxes"],
                crack_output2["labels"],
                roi2.shape,
                W_crack,
                H_crack,
            )
